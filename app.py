...
if selected == 'Карты':
    with st.container():
        ...
        with col1:
            st.pydeck_chart(pdk.Deck(
                ...
                tooltip={
                    "html": "<b>{Вид}</b> <b>{Наименование пункта пропуска}</b>, поток: <b>{Количество}</b> единиц в год",
    ...
    with st.container():
        ...
        categoryy = st.selectbox(
            '**:gray[Выберите категорию, по которой хотите посмотреть перегруженные пропускные пункты:]**',
            ('Число людей', 'Легковые транспортные средства',
             'Грузовые транспортные средства',
        ...
        with col1:
            ...
            st.pydeck_chart(pdk.Deck(
                ...
                tooltip={
                    "html": "<b>{Вид}</b> <b>{Наименование пункта пропуска}</b>, поток превышен в <b>{Количество (Факт/Паспорт)}</b> раз(а)",
                    "style": {"background": "grey", "color": "white", "font-family": '"Helvetica Neue", Arial',
                              "z-index": "10000"},
                    }
            ))
    ...
    st.write('---')
    with st.container():
        st.subheader('***Седневзвешенный центр распределения потоков пропускных пунктов***', divider='blue')
        category_ = st.selectbox(
            '**:gray[Выберите категорию, по которой хотите посмотреть средневзвешенный центр:]**',
            ('Число людей', 'Легковые транспортные средства',
             'Грузовые транспортные средства',
             'Паспорта транспортных средств', 'Грузы в тоннах'))
        st.write('---')
...
if selected == 'Статистика по Федеральным округам':
    ...
    with st.container():
        ...
        if value == 'Распределение пропускных пунктов':
            ...
            st.plotly_chart(fig, theme="streamlit")
            st.write()
            question = st.checkbox('**:red[Какой пункт считается перегруженным]:grey_question::question:**')
            if question:
                st.write('**У каждого пункта пропуска есть максимальная пропускная способность, но есть пропускные пункты, фактическое количество пересечений которых превышает максимально возморжное. Пропускной пункт считается перегруженным, если результат деления фактического количества пересечений на максимально возможно превышает единицу.**')
        ...
        if value == 'Распределение пропускных пунктов':
            ...
            option = st.radio('**:blue[Выберите категорию, по которой хотите посмотреть потоки по пропускным пунктам]**',
                              ('Число людей', 'Легковые транспортные средства',
                               'Грузовые транспортные средства',
                               'Паспорта транспортных средств', 'Грузы в тоннах'))
            fig=px.histogram(places_copy[places_copy['Категория']==option],
            ...
            option = st.radio('**:blue[Выберите категорию, по которой хотите посмотреть потоки по пропускным пунктам]**',
                              ('Число людей', 'Легковые транспортные средства',
                               'Грузовые транспортные средства',
                               'Паспорта транспортных средств', 'Грузы в тоннах'))
            fig=px.histogram(places_copy[places_copy['Категория']==option], x="Количество",
                             y="Сопредельное государство",
            ...
            fig=px.bar(push[(push['Количество (Факт/Паспорт)']>1)&(push['Год']!=2020)&
                            (push['Категория']==option)&
                            (push['Наименование пункта пропуска']!='Благовещенск')],
