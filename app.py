...
def load_model_2(model_name_2):
    ...
    return (all_df_copy)
all_df_copy = load_model_2('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/all_df_copy.csv')
...
def load_model_3(model_name_3):
    ...
    return (people_zero)
people_zero = load_model_3('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/people_zero.csv')
...
def load_model_4(model_name_4):
    ...
    return (places_copy)
places_copy = load_model_4('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/places_copy.csv')
...
def load_model_5(model_name_5):
    ...
    return (push)
push = load_model_5('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/push.csv')
...
controls_else = load_model_6('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/controls_else.csv')
...
def load_model_7(model_name_7):
    ...
    return (places_count)
places_count = load_model_7('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/places_count.csv')
...
rosgran_count = load_model_8('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/rosgran_count.csv')
...
def load_model_9(model_name_9):
    ...
    return (country_else)
country_else = load_model_9('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/country_else.csv')

@st.cache_resource()
def load_model_10(model_name_10):
    Center = pd.read_csv(model_name_10)
    return (Center)
Center = load_model_10('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/Center.csv')

@st.cache_resource()
def load_model_11(model_name_11):
    ANN = pd.read_csv(model_name_11)
    return (ANN)
ANN = load_model_11('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/ANN.csv')

@st.cache_resource()
def load_model_12(model_name_12):
    all_df_copy_na = pd.read_csv(model_name_12)
    return (all_df_copy_na)
all_df_copy_na = load_model_12('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/all_df_copy_na.csv')

@st.cache_resource()
def load_model_13(model_name_13):
    all_df_copy_places = pd.read_csv(model_name_13)
    return (all_df_copy_places)
all_df_copy_places = load_model_13('https://raw.githubusercontent.com/uroplatus666/StreamLit/master/data/all_df_copy_places.csv')
    ...
    with st.container():
        ...
        if value == 'Распределение пропускных пунктов':
            ...
            if option == 'Функционируют:heavy_check_mark:':
                ...
                answer = 'нет'
            fig = px.histogram(people_zero[(people_zero['Год']==year)&(people_zero['Функционирует/не функционирует']==answer)],
                               y="Федеральный округ",
                               color='Вид',
            ...
            st.write('')
            st.subheader('Усредненное количество пересечений пропускных пунктов:chart_with_downwards_trend:', divider='blue')
            option = st.radio('**:blue[Выберите категорию:]**',
                                  ('Число людей', 'Легковые транспортные средства',
                                   'Грузовые транспортные средства',
                                   'Паспорта транспортных средств', 'Грузы в тоннах'))
            ...
            st.write('')
            st.subheader('Процент наличия контроля определенного типа на пропускных пунктах:passport_control:', divider='blue')
            ...
            fig = go.Figure(data=go.Heatmap(z=controls_else[controls_else['Тип контроля']==option]['Процент контроля'],
