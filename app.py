    ...
    with st.container():
        value = st.selectbox(
            ...
            ('Распределение пропускных пунктов',
             ...
             'Пропускные пункты с нулевым фактическим потоком',
             'Сумма потоков пропускных пунктов',
             'Усредненные потоки пропускных пунктов',
        ...
        if value == 'Распределение пропускных пунктов':
            ...
            option = st.radio('**:blue[Выберите категорию, по которой хотите посмотреть потоки по пропускным пунктам]**',
                                  ('Число людей', 'Легковые транспортные средства',
                                   'Грузовые транспортные средства',
                                   'Паспорта транспортных средств'))
            fig=px.bar(all_df_copy[(all_df_copy['Количество (Факт/Паспорт)']>1)&
                                   (all_df_copy['Категория']==option)&
                                   (all_df_copy['Наименование пункта пропуска']!='Благовещенск')],
